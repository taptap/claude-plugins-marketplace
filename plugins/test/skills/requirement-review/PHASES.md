...
当评审发现阻断级问题时，使用 [CONVENTIONS.md ask_question 格式](../../CONVENTIONS.md#ask_question-输出格式) 批量确认。每次 3-5 个阻断项，每个提供 3 个选项：
...
## 阶段 5: output - 输出评审报告并导出飞书文档

**前提**：回读 `requirement_understanding.md` 和 `review_checklist.md`，确保报告内容完整。

### 5.1 组织报告结构

按 [TEMPLATES.md](TEMPLATES.md) 中 report.md 模板的 7 章结构组织报告：

1. 评审结论（Go-No-Go 判定 + 就绪条件 + 元数据）
2. 需求理解（目标 + 功能点清单表）
3. 评审统计摘要（12 维度按 6 大类聚合 + QA Checklist 统计）
4. 各职能待确认问题列表（报告核心，按 PM / Dev / QA / Design 分组）
5. 风险项摘要（按筛选标准从评审发现中提取）
6. 评审范围声明（文档版本 + 信息边界 + 分析模式）
7. 评审完整性声明（统计数字）

### 5.2 导出飞书文档

1. 将精简 Markdown 报告写入 `report.md` 文件
2. 通过 `create_feishu_doc.py` 创建飞书文档（使用 `2>&1` 确保 stderr 可见）：

```bash
python3 $SKILLS_ROOT/shared-tools/scripts/create_feishu_doc.py create \
  --title "XXX需求-需求评审-YYYYMMDDHH" --file report.md \
  --wiki-parent <workflow prompt 中注入的 token> 2>&1
```

3. 在 chat 中返回简要总结和飞书文档链接

失败重试策略见 [shared-tools 通用约定](../shared-tools/SKILL.md#脚本失败重试策略)。3 次尝试后仍失败时在 chat 中输出错误摘要，提示用户手动创建文档后粘贴 report.md 内容。
