...
3. 查找 `ensure-golang.sh`：
   - plugin root: `${CLAUDE_PLUGIN_ROOT}/scripts/ensure-golang.sh`
   ...
   - 优先级：`${CLAUDE_PLUGIN_ROOT} > primary > cache`
4. 运行：
...
建议：
  ...
  - 手动运行：`bash "${CLAUDE_PLUGIN_ROOT}/scripts/ensure-golang.sh" --verbose`（若当前会话可用）
```
