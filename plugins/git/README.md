...
⚙️ **逻辑层** - [command-procedures.md](skills/git-flow/command-procedures.md)
- 任务 ID 提取（三级优先级策略）
- 分支检查逻辑
- Commit 信息生成
- 智能分支前缀判断
...
#### 显式命令调用
```
用户输入: /git:commit (或 -push, -push-pr)
    ↓
Commands (commit.md / commit-push.md / commit-push-pr.md)
    ↓
引用 command-procedures.md 中的共享逻辑:
    ├─→ 分支检查 (#分支检查逻辑)
    ├─→ 任务 ID 提取 (#任务ID提取)
    └─→ Commit 生成 (#commit信息生成规范)
    ↓
参考 reference.md 验证规范
    ↓
执行 Git 操作
```
...
#### 自然语言触发
```
用户输入: "帮我提交" / "commit 一下" / "提交代码"
    ↓
Claude Code 识别关键词
    ↓
触发 git-flow Skill (SKILL.md)
    ↓
引用相同的共享逻辑:
    ├─→ command-procedures.md (#任务ID提取)
    ├─→ command-procedures.md (#commit信息生成规范)
    └─→ reference.md (#提交信息规范)
    ↓
执行 Git 操作
```
...
### 内部依赖
- **规范文档**: [reference.md](skills/git-flow/reference.md)
- **共享逻辑**: [command-procedures.md](skills/git-flow/command-procedures.md)
